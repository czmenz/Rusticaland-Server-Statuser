...
void check_internet(){
    ...
    for (int i=0;i<5;i++){ int ms = http_ping(L"www.google.com", L"/generate_204"); if (ms>=0) samples.push_back(ms); delay_ms(100); }
    if (!samples.empty()){ int avg = (int)std::accumulate(samples.begin(), samples.end(), 0) / (int)samples.size(); cprint("Success", std::string("Network is connected (response time to google.com ") + std::to_string(avg) + " ms)"); }
    else cprint("Error", "Network is not connected");
}
